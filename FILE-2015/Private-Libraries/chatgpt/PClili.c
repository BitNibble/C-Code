...
*/
#define EMPTY 0
#define RMFR 1   // Remove Forward
#define INSFR 1  // Insert Forward
...
/*** record ***/
void LILIrecord(struct lili *l, const char* data) {
    if (l->target == NULL) {  // Initialize list
...
void LILIremove(struct lili *l) {
    ...
    LILIDATA *ptr = l->target;

    // Handle removal cases
    if (l->target->next == l->target->back) { // Only one element
        ...
        l->target = RMFR ? l->target->back : l->target->next;  // Decide direction to move after removal
        l->total--;
        free(ptr);
    }
}
...
void LILIpush(struct lili *l, const char* data) {
    if (l->target == NULL) {  // Initialize list
        ...
        l->target = l->head;  // Move to head
        l->target->next = (LILIDATA*)calloc(1, sizeof(LILIDATA));
        l->target->next->next = l->target->next;
        l->target->next->back = l->target;
        strcpy(l->target->next->vector, data);
        l->target = l->target->next;  // Update target
        l->total++;
    }
}
...
LILIDATA LILIpop(struct lili *l) {
    ...
    LILIDATA *ptr = l->target;
    l->target = l->target->back;  // Move target to previous
    return *ptr;  // Return popped data
}
